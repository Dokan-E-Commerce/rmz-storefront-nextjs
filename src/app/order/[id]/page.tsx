...
export default function OrderDetailPage({ params }: OrderDetailPageProps) {
  ...
  const handleSubmitReview = async (e: React.FormEvent) => {
    ...
    try {
      // Prepare form data to match legacy structure
      ...
      // TODO: Add review submission API method to SDK
      // const response = await ordersApi.submitReview(order.id, reviewData);
                    ...
                    <div key={index} className="bg-card/50 backdrop-blur-sm border border-border/30 rounded-xl p-6 hover:shadow-lg transition-shadow duration-300">
                      <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {item.item?.type === 'code' && ((order.status as any)?.id === 3 || (order.status as any)?.id === 4) && item.codes && item.codes.length > 0 && (
                            <div className="mb-4">
                          ...
                          {/* Custom Fields - matches: {% for key,field in item.fields %} */}
                          {item.fields && typeof item.fields === 'object' && Object.keys(item.fields).length > 0 && (
                            <div className="mb-4">
                              <div className="space-y-1">
                                {Object.entries(item.fields).map(([key, field]: [string, any], fieldIndex: number) => (
                          ...
                          {item.item?.type === 'product' && ((order.status as any)?.id === 3 || (order.status as any)?.id === 4) && item.item?.product?.full_link && (
                            <div className="mb-4">
                              <a
              ...
              {(order as any).transaction && (
                <div className="mt-6 pt-6 border-t border-border/30">
                  ...
                  <div className="space-y-3 text-sm">
                        ...
                        <span className="font-medium text-foreground">{(order as any).transaction.payment_method || t('free')}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">{t('transaction_id')}</span>
                        <span className="font-medium text-foreground">{(order as any).transaction.payment_id || '-'}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">{t('refund_status')}</span>
